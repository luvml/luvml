...
public final class Frags<T extends Frag_I<?>> implements Frags_I<Frags<T>> {
    ...
    /**
     * Conditional fragment. When the condition is false nothing is emitted at all.
     * Attributes are fragments too, so this covers conditional attributes as well as
     * conditional elements - there is no separate attribute form.
     *
     * <pre>
     * div(if_(loggedIn, () -&gt; p("Welcome back")))
     * a(href(u), if_(current, () -&gt; class_("sidebar_on")), text(s))
     * </pre>
     */
    public static Frags_I if_(boolean condition, Supplier<? extends Frag_I> frag){
        if(condition)return frags((Frag_I)frag.get());
        return frags();
    }
